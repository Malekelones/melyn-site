...
export function renderServiceDetail(serviceId) {
  ...
  if (!service) {
    app.innerHTML = `
      <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:1rem;">
        <h1 style="font-size:2rem;font-family:var(--font-display);">${t('detail.notFound')}</h1>
        <a href="#/" class="btn btn-primary">${t('detail.backHome')}</a>
      </div>
    `;
    return;
  }

  const scopeList = localized(service.scope);
  const resultsList = localized(service.results);

  const scopeItems = (Array.isArray(scopeList) ? scopeList : []).map(item => `
  ...
  const resultItems = (Array.isArray(resultsList) ? resultsList : []).map(item => `
    <div class="detail-list-item">
    ...
    <section class="service-hero">
      ...
      <div class="service-hero-content">
        <nav class="breadcrumb reveal-left">
          <a href="#/">${t('detail.breadcrumbHome')}</a>
          <span class="sep">→</span>
          <a href="#/">${t('detail.breadcrumbServices')}</a>
          <span class="sep">→</span>
          <span class="current">${localized(service.title)}</span>
        </nav>
        <h1 class="hero-title" style="font-size:var(--text-4xl);margin-bottom:var(--space-sm);">${localized(service.title)}</h1>
        <p style="color:var(--color-text-secondary);font-size:var(--text-lg);max-width:600px;animation:fade-in-up 1s 0.3s var(--ease-out-expo) backwards;">${localized(service.shortDesc)}</p>
      </div>
    </section>
      ...
      <div class="container">
        <div class="service-detail-grid" style="margin-bottom:var(--space-3xl);">
          <div class="glass-card reveal">
            <h2 style="font-family:var(--font-display);font-size:var(--text-xl);font-weight:600;margin-bottom:var(--space-lg);color:var(--color-cyan);">${t('detail.scope')}</h2>
            <div class="detail-list">
              ${scopeItems}
            </div>
          </div>
          <div class="glass-card reveal delay-2">
            <h2 style="font-family:var(--font-display);font-size:var(--text-xl);font-weight:600;margin-bottom:var(--space-lg);color:var(--color-cyan);">${t('detail.results')}</h2>
            <div class="detail-list">
              ${resultItems}
            </div>
          </div>
        ...
        <div class="reveal" style="margin-bottom:var(--space-3xl);">
          <h3 style="font-family:var(--font-display);font-size:var(--text-lg);font-weight:600;margin-bottom:var(--space-md);">${t('detail.technologies')}</h3>
          <div class="service-card-tags" style="gap:var(--space-sm);">
            ${service.tags.map(tag => `<span class="service-tag" style="font-size:var(--text-sm);padding:0.35rem 0.85rem;">${tag}</span>`).join('')}
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-lg);margin-bottom:var(--space-3xl);">
          ${prevService ? `
            <a href="#/services/${prevService.id}" class="glass-card reveal" style="padding:var(--space-lg);display:flex;align-items:center;gap:var(--space-md);">
              ...
              <div>
                <div style="font-size:var(--text-xs);color:var(--color-text-muted);margin-bottom:0.25rem;">${t('detail.prev')}</div>
                <div style="font-family:var(--font-display);font-weight:600;">${localized(prevService.title)}</div>
              </div>
            </a>
          ` : '<div></div>'}
          ${nextService ? `
            <a href="#/services/${nextService.id}" class="glass-card reveal delay-1" style="padding:var(--space-lg);display:flex;align-items:center;justify-content:flex-end;gap:var(--space-md);text-align:right;">
              <div>
                <div style="font-size:var(--text-xs);color:var(--color-text-muted);margin-bottom:0.25rem;">${t('detail.next')}</div>
                <div style="font-family:var(--font-display);font-weight:600;">${localized(nextService.title)}</div>
              </div>
              <span style="font-size:1.5rem;color:var(--color-cyan);">→</span>
            </a>
          ` : '<div></div>'}
        ...
        <div class="cta-section reveal-scale" style="margin-bottom:var(--space-4xl);">
          <h2 class="cta-title">${localized(service.ctaTitle)}</h2>
          <p class="cta-desc">${localized(service.ctaDesc)}</p>
          <div class="cta-actions">
            <a href="#" class="btn btn-white" id="serviceCtaBtn">${t('detail.contactUs')}</a>
            <a href="https://calendly.com/onesmalek-benamor/30min" target="_blank" rel="noopener" class="btn btn-outline">${t('detail.schedule')}</a>
          </div>
        </div>
      </div>
    </main>
  ...
  // Update page title
  document.title = `${localized(service.title)} — MELYN`;
}
